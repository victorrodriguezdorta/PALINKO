...
abstract class OpenAiCompatibleWordRelationChecker implements WordRelationChecker {
    ...
    private Object chatRequest(String wordA, String wordB, GameLanguage language) {
        ...
        String systemPrompt = "You judge how related two words or short phrases are, for a word-chain party "
                ...
                + "does. Be a FAIR judge: score based on whether a reasonable person would accept the link in "
                + "casual play, not on whether it's the tightest, most textbook-perfect association possible. "
                + "A good link is SPECIFIC to these two exact words, not just two unrelated words that happen "
                + "to share a broad category with nothing else in common. "
                + "Score HIGH (70-100) for genuine, specific, direct associations that most people would "
                + "recognize: synonyms and near-synonyms, direct cause-and-effect pairs (rain / umbrella), "
                + "part-whole pairs (wheel / car), typical co-occurrence (beach / sunscreen), a tool and its "
                + "purpose (needle / sew), opposites (hot / cold), a well-known cultural or pop-culture "
                + "association (Batman / Gotham), or members of the same specific, narrow category (perro / "
                + "gato as pets/animals is a real, recognizable link, not just an abstract category). "
                + "Score MEDIUM (30-69) when there is a real but weaker, more indirect, or more general "
                + "connection you could still name concretely — including a shared specific subcategory or a "
                + "plausible everyday association that isn't the strongest possible link. "
                + "Score LOW (0-29) only when there is essentially no real connection at all, or the only "
                + "link is an extremely broad, near-universal category so wide it says nothing specific about "
                + "either word — e.g. \"garaje\" and \"biblioteca\" both merely being \"a place\" (a category "
                + "that fits almost anything) is LOW, but a narrower shared category with real specific "
                + "overlap should score MEDIUM or higher. Give the benefit of the doubt to plausible, "
                + "everyday associations a player would reasonably make — do not require the single most "
                + "obvious link to exist for a pair to score well. "
