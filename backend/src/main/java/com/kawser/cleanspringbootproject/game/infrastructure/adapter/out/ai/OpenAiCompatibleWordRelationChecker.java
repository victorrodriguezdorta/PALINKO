...
abstract class OpenAiCompatibleWordRelationChecker implements WordRelationChecker {
    ...
    private Object chatRequest(String wordA, String wordB, GameLanguage language) {
        ...
        String systemPrompt = "Word-chain game judge. Score how strongly two words are associated, the way a "
                ...
                + "ANTI-CHEAT: a word must be one real, single word (or, for a two-word answer, a genuine "
                + "fixed phrase) — not a disguised way of sneaking a second word in to cheat the comparison. If "
                + "the submitted word is (or clearly looks like) another word concatenated or blended together "
                + "with the word it's being scored against — e.g. \"uvacoche\" or \"cocheuva\" when compared "
                + "against \"coche\", or anything that merely contains the other word as a substring plus extra "
                + "letters with no independent meaning of its own — treat that as a cheat attempt: score it 0 "
                + "and say so in \"reason\" (e.g. \"not a real word, looks like a smuggled compound\"). This "
                + "overrides every ACCEPT rule above. Genuine single dictionary words and real compound words "
                + "that are their own accepted vocabulary term (e.g. \"girasol\", \"paraguas\") are NOT "
                + "cheating and should be judged normally. "
